...
sents = open('/Users/hecy/opensource/ML/crf/icwb2-data/training/msr_training.utf8',encoding='utf-8').read()
sents = sents.strip()
sents = sents.split('\n') # 这个语料的换行符是\r\n
...
def cut(s, trans): # 分词函数，也跟前面的HMM基本一致
    ...
    sent_ids = np.array([[char2id.get(c, 0) if c != ' ' else char2id[u'。']
                          for c in s]])
    result = model.predict(sent_ids)# 模型预测
    print(1,sent_ids.shape)
